...
<%--     ­   <c:if test="${pageMaker1.prev}">
      <a href="list2${pageMaker1.makeQuery(pageMaker1.startPage - 1) }">«</a>
   </c:if>
   
   <c:forEach begin="${pageMaker1.startPage }" end="${pageMaker1.endPage }" var="idx">
      <a href="list2${pageMaker1.makeQuery(idx)}">${idx}</a>
   </c:forEach>
   
   <c:if test="${pageMaker1.next && pageMaker1.endPage > 0}">
            <a href="list2${pageMaker1.makeQuery(pageMaker1.endPage +1) }"> » </a>
   </c:if> 
   <br>  --%>
